...
defmodule AWS.ECS do
  ...
  *do not* use a load balancer are considered healthy if they are in the
  ...
  that use the EC2 launch type, the **minimum healthy percent** and **maximum
  percent** values are only used to define the lower and upper limit on the
  ...
  maximum percent values are not used, although they are currently visible
  when describing your service.

  Tasks for services that *do not* use a load balancer are considered healthy
  if they are in the `RUNNING` state. Tasks for services that *do* use a load
  ...
  balancer. The default value for a replica service for
